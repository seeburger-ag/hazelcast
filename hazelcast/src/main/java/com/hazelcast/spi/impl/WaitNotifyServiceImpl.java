...
class WaitNotifyServiceImpl implements WaitNotifyService {
    ...
    protected final ConcurrentMap<WaitNotifyKey, Queue<WaitingOp>> mapWaitingOps =
            new ConcurrentHashMap<WaitNotifyKey, Queue<WaitingOp>>(100);
    protected final DelayQueue<WaitingOp> delayQueue = new DelayQueue<WaitingOp>();
    ...
    protected void invalidate(final WaitingOp waitingOp) throws Exception {
    ...
    protected class ExpirationTask implements Runnable {
        ...
        private boolean doRun() throws Exception {
            ...
            while (waitTime > 0) {
                ...
                WaitingOp waitingOp = delayQueue.poll(waitTime, TimeUnit.MILLISECONDS);
                if (waitingOp != null) {
                    if (waitingOp.isValid()) {
